...
export class SaptivaService {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (contentType && contentType.includes("application/json")) {
          ...
          throw new Error(
            `Error en la API de Saptiva: HTTP ${
              response.status
      ...
      if (!contentType || !contentType.includes("application/json")) {
        ...
        throw new Error(
          `La API de Saptiva no devolvió JSON (${contentType}): ${text.substring(
            0,
            100,
          )}...`,
        );
      ...
      console.warn(
        "Respuesta inesperada de Saptiva API:",
        JSON.stringify(data).substring(0, 200),
      );
      return data || "Lo siento, no pude generar una respuesta adecuada.";
    } catch (error) {
  ...
  async generateEmbeddings(text: string): Promise<number[]> {
    try {
      console.log(
        "Generando embeddings para texto con OpenAI como respaldo:",
        text.substring(0, 50) + "...",
      );
      ...
      console.error(
        "Error al generar embeddings con OpenAI como respaldo:",
        error,
      );
      throw error;
    }
